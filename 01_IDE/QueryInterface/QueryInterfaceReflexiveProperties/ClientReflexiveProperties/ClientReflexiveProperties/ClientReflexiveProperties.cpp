#include<windows.h>
#include"ReflexivePropertiesDll.h"
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT iMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		...
		hrSub = pISum->QueryInterface(IID_ISubtract, (void **)&pISubtract);
		if (FAILED(hrSub))
		{
			MessageBox(hwnd, TEXT("ISubtarct Interface can not be obtained"), TEXT("Error"), MB_OK);
			DestroyWindow(hwnd);
		}
		hrSum = pISubtract->QueryInterface(IID_ISum,(void **)&pISumAgain);
		if (SUCCEEDED(hrSum))
		{
			MessageBox(hwnd, TEXT("pISumAgain Interface be obtained"), TEXT("result"), MB_OK| MB_ICONINFORMATION);
		}
		iNum1 = 150;
		iNum2 = 150;
